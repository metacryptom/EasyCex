import { DepthInfo, ExchageOption, ExchageType, MarketFetcher, Pair, QueryDepthOption, QueryDepthResult, Token } from '../../base'
import { PublicClient } from '@liuxingfeiyu/okex-node'
...
export class OkexMarketFetcher implements MarketFetcher {
    ...
    async queryDepth (opt?: QueryDepthOption): Promise<QueryDepthResult> {
      const depth = await this.pbIns.spot().getSpotBook(this.pairName, { size: opt?.size.toString() || '10' /* ,depth:0.1 */ })
